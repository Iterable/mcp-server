...
describe("keys update command", () => {
  ...
  it("updates existing key without changing API key value", async () => {
    answers.push(
      { name: "production" }, // keep same name
      { updateApiKey: false }, // don't update API key
      { doAdvanced: false }, // skip advanced permissions
      { activateNow: false } // don't activate (already active)
    );
  ...
  it("updates key with new API key value", async () => {
    ...
    answers.push(
      { name: "production-updated" }, // new name
      { updateApiKey: true }, // update API key
      { doAdvanced: false },
      { activateNow: false }
    );
  ...
  it("updates permissions when user selects advanced", async () => {
    answers.push(
      { name: "production" },
      { updateApiKey: false },
      { doAdvanced: true }, // enable advanced
      { permFlags: ["pii", "writes", "sends"] }, // enable all perms
      { activateNow: false }
    );
