...
describe("keys add with duplicate detection", () => {
  ...
  it("converts add to update when duplicate API key detected and user confirms", async () => {
    ...
    answers.push(
      { name: "new-name" }, // user enters a name (for the update)
      { updateApiKey: false }, // don't update API key (keep existing)
      { updateExisting: true }, // user confirms update when duplicate detected
      { doAdvanced: false },
      { activateNow: false }
    );
  ...
  it("cancels and returns existing key ID when user declines update", async () => {
    ...
    answers.push(
      { name: "new-name" }, // enter name
      { updateApiKey: false }, // keep existing API key
      { updateExisting: false } // user declines update when duplicate detected
    );
