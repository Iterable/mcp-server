...
export const setupMcpServer = async (): Promise<void> => {
  ...
  try {
    ...
    // 3) If neither env nor existing key chosen, add a new key using interactive flow
    if (!apiKey && !usedExistingKey) {
      ...
      try {
        const newKeyId = await saveKeyInteractive(
          ...
          keySpinner,
          {
            skipRestartNotice: true, // Skip restart notice, setup flow continues
            advanced, // Always pass true or false, never prompt in setup
          }
        );

        // Get the key metadata for MCP config
        const keys = await keyManager.listKeys();
        selectedExistingMeta = keys.find((k) => k.id === newKeyId);
      } catch (error) {
        showError(error instanceof Error ? error.message : "Failed to add key");
        process.exit(1);
      }
    } else if (usedExistingKey) {
      // Using existing key - offer to activate if not already active
      const keyManager = getKeyManager();
      await keyManager.initialize();
      ...
      // Show key details so user knows current permissions
      console.log();
      ...
      // Using API key from environment - use saveKeyInteractive with prefilled API key
      const keyManager = getKeyManager();
      await keyManager.initialize();
      ...
      try {
        const newKeyId = await saveKeyInteractive(
          ...
          {
            skipRestartNotice: true, // Skip restart notice, setup flow continues
            advanced, // Pass --advanced flag to skip permission prompt
            prefilledApiKey: apiKey as string, // Use API key from environment
          }
        );
    ...
    // Step 4: Get permissions from the stored key
    // All paths now use saveKeyInteractive() or read from existing key
    ...
    let mcpEnv: Record<string, string> = {
      ITERABLE_USER_PII: selectedExistingMeta.env?.ITERABLE_USER_PII || "false",
      ITERABLE_ENABLE_WRITES:
        selectedExistingMeta.env?.ITERABLE_ENABLE_WRITES || "false",
      ITERABLE_ENABLE_SENDS:
        selectedExistingMeta.env?.ITERABLE_ENABLE_SENDS || "false",
    };
    ...
    // Select tools if not already specified via flags
    if (tools.length === 0) {
